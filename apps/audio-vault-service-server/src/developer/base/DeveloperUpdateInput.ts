/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { ApiUsageUpdateManyWithoutDevelopersInput } from "./ApiUsageUpdateManyWithoutDevelopersInput";
import { Type } from "class-transformer";
import { LicenseAgreementUpdateManyWithoutDevelopersInput } from "./LicenseAgreementUpdateManyWithoutDevelopersInput";

@InputType()
class DeveloperUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  apiKey?: string | null;

  @ApiProperty({
    required: false,
    type: () => ApiUsageUpdateManyWithoutDevelopersInput,
  })
  @ValidateNested()
  @Type(() => ApiUsageUpdateManyWithoutDevelopersInput)
  @IsOptional()
  @Field(() => ApiUsageUpdateManyWithoutDevelopersInput, {
    nullable: true,
  })
  apiUsages?: ApiUsageUpdateManyWithoutDevelopersInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: () => LicenseAgreementUpdateManyWithoutDevelopersInput,
  })
  @ValidateNested()
  @Type(() => LicenseAgreementUpdateManyWithoutDevelopersInput)
  @IsOptional()
  @Field(() => LicenseAgreementUpdateManyWithoutDevelopersInput, {
    nullable: true,
  })
  licenseAgreements?: LicenseAgreementUpdateManyWithoutDevelopersInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  subscriptionPlan?: string | null;
}

export { DeveloperUpdateInput as DeveloperUpdateInput };
